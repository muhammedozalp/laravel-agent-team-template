...
| Hook | Matcher | Effect |
|---|---|---|
| `block-generated-and-env-edits.sh` | `Write\|Edit` | **DENIES** edits to `vendor/`, `node_modules/`, `public/build/`, `storage/`, `.env*` (allows `.env.example`/`sample`/`template`) |
...
`/hooks`. Headless `claude -p` does **not** load project-directory hooks. The Lead
